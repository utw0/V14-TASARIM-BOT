        ...
        collector.on('collect', async (button, user) => {
            await button.deferUpdate();
                if (button.customId === "ileri") {
                    ...
                    currentPage++;
                    if (msg) msg.edit({ embeds: [embed.setDescription(`${member} adlı üyenin toplam **${luhux - 1}** adet ceza geçmişi bulundu!\n\n${pages[currentPage - 1].map(x => `**#${x.cezaID}** \`[${x.type}]\` <t:${(x.cezaTarih/1000).toFixed()}> <@${x.Staff}>: \`${x.sebep}\``).join("\n")}`)] })
