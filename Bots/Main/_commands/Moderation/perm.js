...
const { EmbedBuilder,PermissionsBitField,Formatters, ActionRowBuilder,StringSelectMenuBuilder, ButtonBuilder, ButtonStyle,ChannelType,PermissionFlagsBits, ActionRow, RoleSelectMenuBuilder  } = require("discord.js");
...
async onRequest (client, message, args,embed) {
    if(!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) return message.react(await emojiBul("appEmoji_carpi"))
    ...
    if(args[0] == "ekle"){
        ...
        if (isim.first().content.includes(" ")) {
          ...
          isim.first().delete();
          await mesaj.edit({content: null, embeds: [embed.setDescription(`Komutu kullanma izni verilcek rolleri aşağıda ki menüden seçiniz?`)], components: [new ActionRowBuilder().setComponents(new RoleSelectMenuBuilder().setCustomId("permRoleSelectMenu").setMaxValues(5))]});
    })
    ...
    collector.on('collect', async i => { 
      ...
      if(i.customId == "permRoleSelectMenu") {
         ...
         let mesajx =await mesaj.channel.send({ embeds: [ embed.setDescription(`Komutun vericeği rolü aşağıdan seçiniz`)],components: [new ActionRowBuilder().setComponents(new RoleSelectMenuBuilder().setCustomId("permRolesSelectMenu").setMaxValues(5))]})
