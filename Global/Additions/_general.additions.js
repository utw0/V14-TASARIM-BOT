...
const cevap = global.cevap = async function(message,type){
  ...
  if(type == "katılım") return message.reply({embeds:[hataEmbed.setDescription(`${hata} **Sunucuda \`katıldı\` Adında Rol Bulunamadı!**`)]}).then(async msg =>{
    setTimeout(async() => {
      if(msg && msg.deletable) await msg.delete();
      if(message && message.deletable) await message.delete();
    }, 10000);
  })
  if(type == "uyarı") return message.reply({embeds:[hataEmbed.setDescription(`${hata} **Sunucuda \`katılmadı\` Adında Rol Bulunamadı!**`)]}).then(async msg =>{
    setTimeout(async() => {
      if(msg && msg.deletable) await msg.delete();
      if(message && message.deletable) await message.delete();
    }, 10000);
  })
  if(type == "mazeret") return message.reply({embeds:[hataEmbed.setDescription(`${hata} **Sunucuda \`mazeret\` Adında Rol Bulunamadı!**`)]}).then(async msg =>{
    setTimeout(async() => {
      if(msg && msg.deletable) await msg.delete();
      if(message && message.deletable) await message.delete();
    }, 10000);
